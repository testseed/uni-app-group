...
const getOrderIndex = ref(0)
const orderList = ref([])
const getOrderList = async () => {
  const res = await getOrderListAPI(getOrderIndex.value)
  console.log(res)
...
onLoad(() => {
  getOrderList()
  getOrderIndex.value = query.type
})
const tabOrder = (orderState: number) => {
  cursorIndex.value = orderState
  getOrderIndex.value = orderState
  getOrderList()
}
</script>
...
<template>
  <view class="viewport">
    ...
    <view class="tabs">
      <text
        class="item"
        @click="tabOrder(item.orderState)"
      ...
      <!-- 游标 -->
      <view class="cursor" :style="{ left: cursorIndex * 20 + '%' }"></view>
    </view>
    <!-- 滑动容器 -->
    <swiper class="swiper">
      <!-- 滑动项 -->
